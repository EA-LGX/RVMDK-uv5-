...
// PC6 片选 PC7 SCK PC8 MOSI PC9 复位  PB6 MISO  CE PA0  IRQ PA1
//初始化24L01的IO口
...
u8 NRF24L01_Write_Reg(u8 reg, u8 value) {
	...
	status = RC522_SPI_ReadWriteOneByte(reg);//发送寄存器号 
	RC522_SPI_ReadWriteOneByte(value);      //写入寄存器的值
...
u8 NRF24L01_Read_Reg(u8 reg) {
	u8 reg_val;
	NRF24L01_CSN = 0;          //使能SPI传输		
	RC522_SPI_ReadWriteOneByte(reg);   //发送寄存器号
	reg_val = RC522_SPI_ReadWriteOneByte(0XFF);//读取寄存器内容
	NRF24L01_CSN = 1;          //禁止SPI传输		    
...
u8 NRF24L01_Read_Buf(u8 reg, u8* pBuf, u8 len) {
	u8 status, u8_ctr;
	NRF24L01_CSN = 0;           //使能SPI传输
	status = RC522_SPI_ReadWriteOneByte(reg);//发送寄存器值(位置),并读取状态值   	   
	for (u8_ctr = 0;u8_ctr < len;u8_ctr++)pBuf[u8_ctr] = RC522_SPI_ReadWriteOneByte(0XFF);//读出数据
...
u8 NRF24L01_Write_Buf(u8 reg, u8* pBuf, u8 len) {
	...
	NRF24L01_CSN = 0;          //使能SPI传输
	status = RC522_SPI_ReadWriteOneByte(reg);//发送寄存器值(位置),并读取状态值
	for (u8_ctr = 0; u8_ctr < len; u8_ctr++)RC522_SPI_ReadWriteOneByte(*pBuf++); //写入数据	 
	NRF24L01_CSN = 1;       //关闭SPI传输
	return status;          //返回读到的状态值
}
...
//该函数初始化NRF24L01到TX模式  // _TX_ADDRESS:发送地址 
...
void NRF24L01_TX_Mode(u8* TX_ADDRESS, u8* RX_ADDRESS) {
	...
	NRF24L01_Write_Buf(NRF_WRITE_REG + RX_ADDR_P0, RX_ADDRESS, RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  
