...
void LCD_ShowString(u16 x, u16 y, char* p) {//显示一个字符串         
	while (*p != '\0') {
...
// 加载进度条
void LCD_LoadBar(u16 x, u16 y, u16 width, u16 height, u8 percent) {
	u16 i, j;
	u16 x0 = x;
	u16 y0 = y;
	u16 x1 = x + width - 1;
	u16 y1 = y + height - 1;
	Address_set(x0, y0, x1, y1);      //设置光标位置 
	for (i = y0;i <= y1;i++) {
		for (j = x0;j <= x1;j++) {
			if (j < x0 + width * percent / 100) {
				LCD_WR_DATA(POINT_COLOR);
			}
			else {
				LCD_WR_DATA(BACK_COLOR);
